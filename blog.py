...
def index():
    ...
    return render_template("index.html",numbers = articles)
@app.route("/about")
def about():
...
def delete(id):
    ...
    if result > 0:
        ...
        flash("Böyle bir makale bulunamadı veya bu işleme yetkiniz yoktur.","danger")
        return redirect(url_for("index"))
...
def update(id):
    if request.method == "GET":
        ...
        if result == 0 :
            flash("Bu makale görüntülenemiyor.","danger")
            return redirect(url_for("index"))
        else :
            article = cursor.fetchone()
            form = ArticleForm()
